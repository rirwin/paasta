...
def spotfleet_metrics_provider(spotfleet_request_id, resource, pool_settings):
    ...
    if not sfr or sfr['SpotFleetRequestState'] == 'cancelled':
        ...
        return 0, 0
    elif sfr['SpotFleetRequestState'] == 'cancelled_running' or sfr['SpotFleetRequestState'] == 'active':
...
def get_spot_fleet_delta(resource, error):
    ...
    current_capacity = int(spot_fleet_request['SpotFleetRequestConfig']['FulfilledCapacity'])
...
def is_resource_cancelled():
    if resource['type'] == 'aws_spot_fleet_request':
        state = get_sfr(spotfleet_request_id, region=resource['region'])['SpotFleetRequestState']
...
def autoscale_local_cluster(dry_run=False):
    ...
    for resource in autoscaling_resources:
        resource['cancelled'] = is_resource_cancelled(resource)
